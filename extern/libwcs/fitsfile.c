...
/* == Added by Image Marker == */

#ifdef _WIN32
#include <io.h> /* use io.h instead of unistd.h if on Windows */
#define STDIN_FILENO = _fileno(stdin); /* alias for Windows */
#define access _access /* alias for Windows */

#else
#include <unistd.h> /* use unistd.h if on Unix */
#endif
